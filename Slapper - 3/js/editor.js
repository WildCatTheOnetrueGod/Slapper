 ...
 * ⚠ 唯一修改處：檔案最下方的初始化觸發，從 DOMContentLoaded 改為等待
 *    window.workspacePanelReady (因為 #realNovelInput 等元素現在是透過
 *    00shared.js 的 fetch 非同步注入，DOMContentLoaded 當下可能還不存在)。
 *    其餘所有邏輯與原版完全相同，可直接被 01.html / 02.html 共用。
 */
...
(function () {
  ...
  let backToTopBtn, backToBottomBtn;
  ...
  function init() {
    ...
    backToBottomBtn = document.getElementById('backToBottomBtn');
    ...
    if (backToBottomBtn) {
      backToBottomBtn.addEventListener('click', () => {
        if (realNovelInput) {
          realNovelInput.scrollTop = realNovelInput.scrollHeight;
          if (backdrop) backdrop.scrollTop = backdrop.scrollHeight;
        } else {
