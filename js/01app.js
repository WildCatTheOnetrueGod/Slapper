...
window.readmeDict = window.readmeDict || {
  text: {
    ...
    <ol>
      ...
      <li>點擊「readme.md」下方的「autoSave」可取消/確認是否自動儲存文檔。(意即，網頁關閉時會將文檔暫存至本機，下次開啟時載入)</li>
...
2026-09-05 00:03:54:058 : 21, LastWhiteID_EMV=6830048`;
  ...
  /**
   * 更新透明度函式
   * @param {number} value - 滑桿的值 (0 ~ 100)
   */
  function updateOpacity(value) {
    if (!editorWrapper) return;

    // 將 0~100 的數值轉換為 CSS opacity 的 0~1 範圍
    const opacityLevel = value / 100;
    
    // 設定 opacity 會自動作用於 .editor-wrapper 及其內部所有子元件
    editorWrapper.style.opacity = opacityLevel;

    // 若有顯示數值的標籤，同步更新
    if (opacityValueText) {
      opacityValueText.textContent = `${value}%`;
    }
  }
